...
/**
 * Created by Disi Ji on 15/12/3.
 */
public class Kmeans {
    private ArrayList<double[]> centers = new ArrayList<double[]>();
    private int dim = 100;
    public Kmeans(ArrayList<double[]> centers){
    ...
    public int[] classify(double[] pattern){
        int cata = 0;
        double min_dist = calculateDist(pattern,this.centers[0]);
        for(int i=0; i<this.centers.size();i++){
            double dist = calculateDist(pattern,this.centers[ij]);
            if(dist < min_dist){
                cata = i;
                min_dist = dist;
            }
